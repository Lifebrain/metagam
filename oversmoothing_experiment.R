library(furrr)
plan(multisession)
metafits <- future_map(1:30, function(i){
  ...
  metafits <- lapply(
    ...
    function(x){
      models <- lapply(list(dat1, dat2, dat3), function(d){
        ...
        singlegam(model, nmc = 1000, grid_length = 100, freq = x[[1]], unconditional = x[[2]])
      })
      metafit <- metagam(models)

      # Fit with full data. Need this each time because of grid
      fullfit <- mgcv::gam(y ~ s(x2), data = dat, method = "REML")
      pred_df <- metafit$all_posteriors$x2[metafit$all_posteriors$x2$quantity == "median", "predictor_value", drop = FALSE]
      names(pred_df) <- "x2"
