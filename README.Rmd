...
Now assume that the objects `fit_no_raw1`, `fit_no_raw2`, and `fit_no_raw3` have been gathered in a single location. We can now perform a meta-analysis of these fits using the `metagam()` function. We gather them in a list:
...
The `plot_heterogeneity()` function visualizes the heterogeneity of the separate fits as measured by the Q-statistic of Cochrane's Q test:
...
The `plot_domination()` function measures the relative contribution of each dataset along the x-axis:
