...
`metagam` has two main functions: `strip_rawdata` and `metagam`. To illustrate them, we start by simulating three datasets. We need the to load the `mgcv` package in order to fit GAMs.
...
First we fit a separate GAM to each dataset. In the typical application of `metagam`, this will be done separately in the location of each dataset. The function `strip_rawdata` removes all subject-specific data from the model object, so that it can be shared to a common location while protecting privacy.
