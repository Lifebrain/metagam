...
#' @param intercept Logical defining whether or not to include the intercept in
#'   each smooth term. Only applies when \code{type = "iterms"}.
...
metagam <- function(models, grid = NULL, grid_size = 100, type = "iterms", terms = NULL,
  ...
  if(!is.null(nsim)){
    ...
    masd_list <- lapply(seq_along(models), function(ind){
      getmasd(models[[ind]], grid, nsim, terms, intercept_se = (type == "iterms"))
    })
